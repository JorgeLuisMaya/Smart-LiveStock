...
class SmartLivestockSupabaseUser extends BaseAuthUser {
  SmartLivestockSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> smartLivestockSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = SmartLivestockSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
